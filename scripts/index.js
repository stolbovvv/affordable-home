...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function initGallery(galleryClass) {
    const gallery = document.querySelectorAll(".".concat(galleryClass));
    if (!gallery || gallery.length === 0) return;
    gallery.forEach(item => {
      const descr = item.querySelector(".".concat(galleryClass, "__descr-wrapper"));
      const viewer = item.querySelector(".".concat(galleryClass, "__photo-viewer"));
      const thumbs = item.querySelectorAll(".".concat(galleryClass, "__thumbnails a"));
      const progress = item.querySelector(".".concat(galleryClass, "__progress"));
      const prev = document.querySelector(".".concat(galleryClass, "__button-prev"));
      const next = document.querySelector(".".concat(galleryClass, "__button-next"));
      ...
      thumbs.forEach((item, index) => {
        ...
        const inf = document.querySelector(".".concat(galleryClass, "-descr"));
        img.src = item.href;
        img.style['opacity'] = 0;
        images.push(img);
        viewer.append(img);
